...
- **One LLM call per run.** No agent loop, no tool-calling. `chat.completions.parse`
  with `response_format=AuditRemediationReport` is the whole contract.
...
- Config only through `app.core.config.Settings` (pydantic-settings, `AUDITOR_`
  prefix; `OPENAI_API_KEY` / `GITHUB_*` are un-prefixed aliases).
