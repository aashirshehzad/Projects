...
_SYSTEM_PROMPT = (
    ...
    "Each scenario leg also needs investment_impact: a short, concrete "
    "portfolio-level phrase (not a paragraph) stating what that leg means in "
    "practice, e.g. 'Revenue growth >10% & margin expansion' or 'Margin "
    "compression below 25% gross'.\n"
    "Also produce basket-level cross_cutting_risks.\n"
    "4. Key insights: 3-5 short, scannable bullet points (key_insights) that "
    "distill the executive thesis for a reader skimming a dashboard — pull out "
    "the single most decision-relevant fact from each of price action, "
    "fundamentals, and news/catalysts. These are a different, denser view of "
    "the thesis, not a restatement of downside_risks.\n"
    "5. Confidence: set confidence to 'High', 'Medium', or 'Low' based on how "
    "well the quantitative (Agent 1), fundamental (Agent 2), and news (Agent "
    "2.5) signals corroborate each other for that ticker. Use 'High' when the "
    "technical picture, fundamentals, and news catalysts all point the same "
    "direction; 'Low' when signals conflict, or when fundamental/news data is "
    "sparse or missing; 'Medium' otherwise.\n"
    "\n"
